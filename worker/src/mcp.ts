...
export function createMcpServer() {
  ...
  server.tool(
    ...
    async ({ secret, title, contact_id, room_id, workspace_id, description, priority, owner, due, start_date, group, depends_on, sequence, estimate, context_ref, metadata }) => {
      const result = await relay("/tasks", { method: "POST", secret, body: { contact_id, room_id, workspace_id, title, description, priority, owner, due, start_date, group, depends_on, sequence, estimate, context_ref, metadata } });
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ secret, contact_id, room_id, workspace_id, task_id, claimed_files, ttl_seconds, reason, force, expected_status, announce, announcement }) => {
      ...
      const result = await relay(`/tasks/${scopeId}/${task_id}/claim`, { method: "POST", secret, body: { claimed_files, ttl_seconds, reason, force, expected_status, announce, announcement } });
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ secret, contact_id, room_id, workspace_id, task_id, summary, status, files_changed, commands_run, verification, blocker, next_step, announce, announcement }) => {
      const scopeId = contact_id || room_id || workspace_id;
      if (!scopeId) return { content: [{ type: "text", text: "Error: contact_id, room_id, or workspace_id is required." }], isError: true };
      const result = await relay(`/tasks/${scopeId}/${task_id}/checkpoint`, { method: "POST", secret, body: { summary, status, files_changed, commands_run, verification, blocker, next_step, announce, announcement } });
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ secret, contact_id, room_id, workspace_id, task_id, to_agent, summary, next_action, status, announce, announcement }) => {
      const scopeId = contact_id || room_id || workspace_id;
      if (!scopeId) return { content: [{ type: "text", text: "Error: contact_id, room_id, or workspace_id is required." }], isError: true };
      const result = await relay(`/tasks/${scopeId}/${task_id}/handoff`, { method: "POST", secret, body: { to_agent, summary, next_action, status, announce, announcement } });
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ secret, action, room_id, task_id, name, runtime, relationship, collaboration_role, ttl_seconds, expires_at, metadata, claim_token, owner, webhook_url, profile_role, runtime_session_ref, delegation_id, reason }) => {
      if (action === "claim") {
        const result = await relay("/delegations/claim", { method: "POST", body: { claim_token, name, owner, webhook_url, profile_role, runtime_session_ref, metadata } });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      if (!secret) return { content: [{ type: "text", text: "Error: secret is required." }], isError: true };
      if (action === "create") {
        const result = await relay("/delegations", { method: "POST", secret, body: { room_id, task_id, name, runtime, relationship, collaboration_role, ttl_seconds, expires_at, metadata } });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      if (action === "list") {
        const params = new URLSearchParams();
        ...
        const query = params.toString();
        const result = await relay(`/delegations${query ? `?${query}` : ""}`, { secret });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      if (action === "revoke") {
        const result = await relay(`/delegations/${delegation_id}`, { method: "DELETE", secret, body: { reason } });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
  ...
  server.tool(
    ...
    async ({ secret, room_id }) => {
      const result = await relay(`/rooms/${room_id}/state`, { secret });
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
