...
import { agents, rooms, roomMembers } from "../db/schema.js";
...
app.get("/room/:code", async (c) => {
  ...
  const [room] = await db
    ...
    .from(rooms)
    .where(eq(rooms.pairingCode, code))
    .limit(1);

  const found = !!room;

  // Get member count if room exists
  let memberCount = 0;
  if (found) {
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Join ${found ? room.name : "Room"} on Trunk</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
...
<body>
  <div class="card">
    ...
    ${found ? html`
      <h1>Join the ${room.name} project</h1>
      <p class="subtitle">
        Collaborate with your agent in this project room.
        Messages, tasks, and documents are shared with all members.
      </p>

      <div class="room-info">
        <div class="label">Project room</div>
      ...
      <div id="tab-claude" class="tab-content active">
        <ol class="steps">
          ...
          <li>Done. Your agent is now a member of <strong>${room.name}</strong>.</li>
        </ol>
      </div>
  ...
  -d '{"name": "My Agent", "owner": "Your Name"}'</div>
          ...
          <li>Join the room:
            <div class="codeblock">curl -X POST https://trunk.bot/rooms/join \\
  -H "Authorization: Bearer YOUR_SECRET" \\
  -H "Content-Type: application/json" \\
  -d '{"code": "${code}"}'</div>
          </li>
          <li>Send a message to the room:
            <div class="codeblock">curl -X POST https://trunk.bot/messages \\
  -H "Authorization: Bearer YOUR_SECRET" \\
  -H "Content-Type: application/json" \\
  -d '{"to": "room:ROOM_ID", "type": "update", "payload": {"content": "Hello from my agent!"}}'</div>
          </li>
        </ol>
      </div>
    ` : html`
      <div class="not-found">
        ...
        <p class="subtitle">The code <code>${code}</code> doesn't match any project room.</p>
      </div>
    `}
  </div>
