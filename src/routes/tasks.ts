...
import { agents, contacts, tasks } from "../db/schema.js";
...
app.post("/", async (c) => {
  ...
  const [task] = await db
    ...
    .values({
      ...
      owner: body.owner || body.contact_id, // default: assign to the other agent
      createdBy: agentId,
      due: body.due,
...
// Update a task
app.patch("/:contactId/:taskId", async (c) => {
  const agentId = c.get("agentId");
  const contactId = c.req.param("contactId");
