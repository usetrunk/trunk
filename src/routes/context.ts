...
app.get("/:contactId/facts/:key", async (c) => {
  ...
  return c.json({ key: fact.key, value: fact.value, updated_by: fact.updatedBy, updated_at: fact.updatedAt });
});
...
app.put("/:contactId/facts/:key", async (c) => {
  ...
  if (existing.length > 0) {
    await db
      .update(sharedFacts)
      .set({ value: body.value, updatedBy: agentId, updatedAt: new Date() })
      .where(and(eq(sharedFacts.scope, scope), eq(sharedFacts.key, key)));
  } else {
    await db.insert(sharedFacts).values({ scope, key, value: body.value, updatedBy: agentId });
  }
  await audit(agentId, "fact.upsert", "shared_fact", `${scope}:${key}`, { contact_id: contactId, key });
  return c.json({ key, value: body.value, updated_by: agentId });
});
